...
double initpricearray()
{
   ...
   if ((OrderTakeProfit() != tpprice)&&(OrderType() == OP_BUY)&&(tpprice > 0.5)&&(OrderOpenPrice()<tpprice)&&(pricehole<0))
   {
...
void checkrange()
{
   ...
   if (GlobalVariableCheck("USDCHF high (simple)"))
   {
      pricehigh = GlobalVariableGet("USDCHF high (simple)");
   }
   
   if (GlobalVariableCheck("USDCHF low (simple)"))
   {
      pricelow = GlobalVariableGet("USDCHF low (simple)");
   }
}
