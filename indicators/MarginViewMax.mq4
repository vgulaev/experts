...
string ExtName="MarginViewMax";
string forex_symbols[255];
string client_symbols[255];
double client_volume[255];
double max_margins[255];
double max_margins_in_moment[255];

double client_symbol_night_margin[255];
int symbol_begin[255];
int symbol_end[255];
...
int table_margins_begin[1024];
int table_margins_end[1024];
//+------------------------------------------------------------------+
...
//----
  ...
  bool TimeInPeriod(int cur_time,int start,int finish)
  {
      if(finish==0 && start == 0) return (false);
      if(finish==0 && start !=0) finish = 86400;
      if(finish<start)
      {
         if(cur_time <finish && cur_time >start) return (true);
         else return (false);
      }
      else
      {
         if(cur_time >=start && cur_time < finish) return (true);
         else return (false);
      }      
  }
...
int start()
  {
   for(int a = 0; a< 255;a++)
   {
      ...
      client_volume[a] = 0;    
      max_margins[a] = 0;  
      max_margins_in_moment[a] = 0;
   }
   int windex=WindowFind(ExtName);
   if(windex>0) ObjectsDeleteAll(windex);
   if(windex<0) return;
   int handle=FileOpen("margin_night.csv",FILE_READ|FILE_CSV,';');
   if(handle<0) 
   {
   ...
   while(!FileIsEnding(handle))
   {
       ...
       string hour = StringSubstr(ss,0,2);
       string minute = StringSubstr(ss,3,2);
       string second = StringSubstr(ss,6,2);
       
       table_margins_begin[i] = StrToInteger(hour)*3600+StrToInteger(minute)*60+StrToInteger(second);
       
       
       
       ss = FileReadString(handle);
       
       hour = StringSubstr(ss,0,2);
       minute = StringSubstr(ss,3,2);
       second = StringSubstr(ss,6,2);
       table_margins_end[i] = StrToInteger(hour)*3600+StrToInteger(minute)*60+StrToInteger(second);
         
         string beg = DoubleToStr(table_margins_begin[i],0);
         string e = DoubleToStr(table_margins_end[i],0);
         assa = assa + beg + " 4."+e;
   ...
   if(ObjectCreate(name,OBJ_LABEL,windex,0,0))
   {
      ...
      ObjectSet(name,OBJPROP_YDISTANCE,ExtVertShift*(1));
      ObjectSetText(name,"Night Margin Max",9,"Arial",ExtColor);
   }
   name = "HEADS"+5;
   if(ObjectCreate(name,OBJ_LABEL,windex,0,0))
   {
   ...
   double maximum_night_margins = 0;
   ...
   for(i=0;i<total_orders;i++)
   {
         ...
         for(j=0;j<count_symbols;j++)
         {
            ...
            if(len+2 == len2)
            {
               ...
               if(symbol_tmp==table_symbols[j])
               {
                  ...
                  symbol_begin[i] = table_margins_begin[j];
                  symbol_end[i] = table_margins_end[j];
                  break;
               }                         
            }
         }     
         ...
         s = s + tmp;     
         max_margins[i] = margin_night;
         //if(maximum_night_margins<margin_night) maximum_night_margins=margin_night;
   ...
   for(i=0;i<total_orders;i++)
   {
      int cur_time = symbol_begin[i];
      for(j=0;j<total_orders;j++)
      {         
         if(TimeInPeriod(cur_time,symbol_begin[j],symbol_end[j]))
         {
              max_margins_in_moment[i] += max_margins[j];
         }
      }
      if(maximum_night_margins<max_margins_in_moment[i]) maximum_night_margins=max_margins_in_moment[i];
      //Print(max_margins_in_moment[i]);
   }
   string summ_night = DoubleToStr(maximum_night_margins,2);
   ...
   if(ObjectCreate(name,OBJ_LABEL,windex,0,0))
   {
      ObjectSet(name,OBJPROP_XDISTANCE,ExtShifts[3]);
      ObjectSet(name,OBJPROP_YDISTANCE,ExtVertShift*(total_orders+5));
      ObjectSetText(name,DoubleToStr(AccountMargin()+StrToDouble(summ_night)-summ_day_margin,2),9,"Arial",ExtColor);
   }
   
   name="Head_"+total_orders+3+"col" + (i+3);
   double night_level = AccountEquity()/(AccountMargin()+StrToDouble(summ_night)-summ_day_margin)*100;  
