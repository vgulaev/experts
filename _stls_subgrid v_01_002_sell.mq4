...
double findholeinpricearray()
{
   ...
   if ((price[pos] != price[pos+1])&&(price[pos+1]<300)&&(NormalizeDouble(price[pos]-lPoint, Digits) != NormalizeDouble(price[pos+1],Digits)))
      {
...
double initpricearray()
{
   ...
   if ((OrderType() % 2 == 0)) continue; //escape all buy orders
   if (StringLen(OrderComment()) != 20) continue;
   if (checkstrategy(OrderComment()) == false) continue;
   if (((OrderOpenPrice() < pricelow-Point)||(OrderOpenPrice() > pricehigh+Point))&&(OrderType()!=OP_SELL))
   {
   ...
   if ((OrderType() == OP_SELL)&&(OrderOpenPrice() > lowbuyprice))
   {
   ...
   /*
   if ((OrderTakeProfit() != tpprice)&&(OrderType() == OP_SELL)&&(tpprice > 0.2)&&(OrderOpenPrice()>tpprice)&&(pricehole < 0)&&(OrderOpenPrice()<(Bid-lPoint*15)))
   {
...
void makeorder(double openprice)
{
   ...
   RefreshRates();
   if (MathAbs(openprice - Bid) > MarketInfo(Symbol(), MODE_STOPLEVEL) * Point)
   {
   ticket = OrderSend(Symbol(), op, lotsize, openprice, 0, 0, 0, ordername);
   Print("Open on price: ", openprice);
   }
   //Print("try", MarketInfo(Symbol(), MODE_STOPLEVEL));
